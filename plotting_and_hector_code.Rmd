...
# Run with GCP NBP
coreGCP <- hector::newcore(ini_file, name = 'GCP_NBPraw')
setvar(coreGCP,gcp_data$year,"NBP_constrain",gcp_data$nbp_raw*0.001,"Pg C/yr")

hector::run(coreGCP,runtodate = 2005)
outGCP  <-fetchvars(coreGCP,dates=1750:2005,vars=c(GLOBAL_TAS(),CONCENTRATIONS_CO2()))
...
ggplot(outGCP) + geom_line(aes(x = year, y = value, color = scenario)) +  facet_wrap(~variable, scales = 'free')
```
