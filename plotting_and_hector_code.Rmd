...
data_dir <- 'results/plot_data_14Mar_2024/'
figure_dir <- 'results/plot_data_14Mar_2024/figures/'
```
...
bind_rows(read.csv(paste0(data_dir, 'uc_plot_data.csv'),
                   stringsAsFactors = F) %>%
          ...
          read.csv(paste0(data_dir, 'coup_plot_data.csv'),
                   stringsAsFactors = F) %>%
            select(year, region, landleaf, name, scenario,tot_land_to_atm_emiss) %>%
            rename(value=tot_land_to_atm_emiss)%>%
            mutate(variable = 'tot_land_to_atm_emiss')) ->
...
#coupled vs uncoupled (C-cycling, Beta, Q10, and Coupled all set to false)
